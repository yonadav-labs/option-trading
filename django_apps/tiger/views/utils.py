...
def get_valid_contracts(ticker, request, use_as_premium, all_expiration_timestamps, filter_low_liquidity):
    ...
    for ts in input_expiration_timestamps:
        ...
        call_lists.append(list(filter(lambda call: call.last_trade_date and not is_low_liquidity(call), calls)))
        put_lists.append(list(filter(lambda put: put.last_trade_date and not is_low_liquidity(put), puts)))
