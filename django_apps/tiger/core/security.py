...
def get_call_puts_from_response(ticker, response, expiration_timestamp=None, external_cache_id=None):
    if 'chain' in response:
        return get_call_puts_intrinio(ticker, response, external_cache_id)
    else:
        return get_call_puts_td(ticker, response, expiration_timestamp, external_cache_id)
