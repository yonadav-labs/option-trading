...
import {
    ...
    useMediaQuery, FormControl, Select, MenuItem, Fab, Toolbar, useTheme,
    Skeleton,
...
import { PriceFormatter, GetGaEventTrackingFunc, TimestampDateFormatter, fixedFloat } from '../../utils';
...
export default function MainView(props) {
    ...
    const [strategySettings, setStrategySettings] = useState({
        "target_price_lower": basicInfo.latestPrice ? fixedFloat(basicInfo.latestPrice * sentiment) : 0,
        "target_price_upper": basicInfo.latestPrice ? fixedFloat(basicInfo.latestPrice * sentiment) : 0,
    });
    ...
    useEffect(() => {
        if (!selectedTicker) {
            setStrategySettings(
                {
                    "target_price_lower": basicInfo.latestPrice ? fixedFloat(basicInfo.latestPrice) : 0,
                    "target_price_upper": basicInfo.latestPrice ? fixedFloat(basicInfo.latestPrice) : 0,
                }
            );
            setContractFilters({});
            setTradeFilters({});
                            ...
                            <Grid item>
                                {renderedTrades.length > 0 ?
                                    ...
                                    loading ?
                                        ...
                                        :
                                        (selectedTicker ?
                                            (selectedExpirationTimestamp && selectedExpirationTimestamp !== 'none' ?
                    ...
                    {loading ?
                        <Grid container justifyContent="center">
                            <Stack paddingX={3} spacing={2} width="inherit">
                                {[1, 2, 3].map(i => <Skeleton animation="wave" variant="rectangular" width="inherit" height="33vh" />)}
                            </Stack>
                        </Grid>
                        :
                        renderedTrades.length > 0 &&
