...
export default function FilterItems(props) {
    ...
    const [targetPrice, setTargetPrice] = useState([(isEmpty(basicInfo) ? 0 : basicInfo.latestPrice), (isEmpty(basicInfo) ? 0 : basicInfo.latestPrice)]);
    ...
    const setDefaultStates = (strategySettings, contractFilters, tradeFilters) => {
        ...
        setTargetPrice([strategySettings['target_price_lower'] || (isEmpty(basicInfo) ? 0 : basicInfo.latestPrice), strategySettings['target_price_upper'] || (isEmpty(basicInfo) ? 0 : basicInfo.latestPrice)]);
    ...
    const clearAllFilters = () => {
        setStrategySettings({
            "target_price_lower": isEmpty(basicInfo) ? 0 : basicInfo.latestPrice,
            "target_price_upper": isEmpty(basicInfo) ? 0 : basicInfo.latestPrice,
        });
        setContractFilters({});
        setTradeFilters({});
    }
